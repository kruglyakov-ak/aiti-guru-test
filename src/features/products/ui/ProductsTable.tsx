import {
  Table,
  TableHeader,
  TableRow,
  TableHead,
  TableBody,
  TableCell,
} from "@/shared/ui/table";
import { Button } from "@/shared/ui/button";
import { Checkbox } from "@/shared/ui/checkbox";
import type { Product, SortDirection, SortField } from "@/shared/types/product";
import { ProductNameCell } from "./ProductNameCell";
import { Plus, MoreHorizontal } from "lucide-react";
import { formatPriceParts } from "@/shared/lib/formatPriceParts";
import { Skeleton } from "@/shared/ui/skeleton";
import { cn } from "@/shared/lib/css";

const columns = [
  { key: "select", label: "", sortable: false },
  { key: "title", field: "title", label: "Наименование", sortable: true },
  { key: "brand", field: "brand", label: "Вендор", sortable: true },
  { key: "sku", field: "sku", label: "Артикул", sortable: true },
  { key: "rating", field: "rating", label: "Оценка", sortable: true },
  { key: "price", field: "price", label: "Цена", sortable: true },
  { key: "actions", label: "", sortable: false },
] as const;

const checkboxClass =
  "w-5 h-5 rounded border-2 transition-colors shrink-0 " +
  "data-[state=checked]:bg-[#3C538E] data-[state=checked]:border-[#3C538E] " +
  "data-[state=unchecked]:bg-white data-[state=unchecked]:border-[#D0D0D0] " +
  "hover:data-[state=unchecked]:border-[#3C538E] [&>span]:hidden";

const PriceCell = ({ price }: { price: number }) => {
  const { integer, fraction } = formatPriceParts(price);

  return (
    <span className="text-[#202020] text-sm font-medium">
      {integer}
      <span className="text-[#999] text-sm">,{fraction}</span>
    </span>
  );
};

const ProductsTableSkeleton = () => (
  <>
    {Array.from({ length: 5 }).map((_, i) => (
      <TableRow key={i}>
        <TableCell colSpan={columns.length} className="p-0">
          <Skeleton className="h-18.25 w-full" />
        </TableCell>
      </TableRow>
    ))}
  </>
);

interface Props {
  products: Product[];
  sortBy: SortField;
  sortDir: SortDirection;
  onSort: (field: SortField) => void;
  selectedIds: number[];
  onSelectRow: (id: number, checked: boolean) => void;
  onSelectAll: (checked: boolean) => void;
  isLoading: boolean;
}

export const ProductsTable = ({
  products,
  sortBy,
  sortDir,
  onSort,
  selectedIds,
  onSelectRow,
  onSelectAll,
  isLoading,
}: Props) => {
  const selectedSet = new Set(selectedIds);
  const allSelected =
    products.length > 0 && selectedIds.length === products.length;

  const someSelected =
    selectedIds.length > 0 && selectedIds.length < products.length;

  const getSortIcon = (field?: SortField) => {
    if (!field || sortBy !== field) return null;
    return sortDir === "asc" ? "↓" : "↑";
  };

  return (
    <Table>
      <TableHeader>
        <TableRow className="border-b border-[#F0F0F0] hover:bg-transparent">
          {columns.map((col, i) => (
            <TableHead
              key={col.key}
              className={cn(
                col.sortable ? "cursor-pointer select-none" : "",
                "p-0 py-4 pl-4.5 pr-6 text-xs font-medium text-[#999] uppercase tracking-wide text-center",
                i === 1 ? "text-left" : "",
              )}
              onClick={
                col.sortable ? () => onSort(col.field as SortField) : undefined
              }
            >
              {col.key === "select" ? (
                <Checkbox
                  checked={
                    allSelected ? true : someSelected ? "indeterminate" : false
                  }
                  onCheckedChange={(v) => onSelectAll(v === true)}
                  className={checkboxClass}
                />
              ) : (
                <>
                  {col.label}
                  {"field" in col && (
                    <span className="text-xs">{getSortIcon(col.field)}</span>
                  )}
                </>
              )}
            </TableHead>
          ))}
        </TableRow>
      </TableHeader>

      <TableBody>
        {isLoading ? (
          <ProductsTableSkeleton />
        ) : products.length === 0 ? (
          <TableRow>
            <TableCell colSpan={columns.length} className="text-center py-8">
              Товары не найдены
            </TableCell>
          </TableRow>
        ) : (
          products.map((product) => {
            const isSelected = selectedSet.has(product.id);

            return (
              <TableRow
                key={product.id}
                className="border-b border-[#F0F0F0] hover:bg-[#FAFAFA]"
              >
                <TableCell
                  className="py-4 pl-4.5 w-10"
                  style={{
                    boxShadow: isSelected ? "inset 4px 0 0 #3C538E" : undefined,
                  }}
                >
                  <Checkbox
                    checked={isSelected}
                    onCheckedChange={(v) => onSelectRow(product.id, v === true)}
                    className={checkboxClass}
                  />
                </TableCell>

                <TableCell className="py-4 pl-4.5 px-4.5 w-125">
                  <ProductNameCell product={product} />
                </TableCell>

                <TableCell className="py-4 pl-4.5 pr-6 text-sm font-bold text-center w-87.5">
                  {product.brand}
                </TableCell>

                <TableCell className="py-4 pl-4.5 pr-6 text-sm text-center ">
                  {product.sku}
                </TableCell>

                <TableCell className="py-4 pl-4.5 pr-6 text-sm text-center ">
                  <span
                    className={
                      product.rating < 3 ? "text-red-500" : "text-black"
                    }
                  >
                    {product.rating.toFixed(1)}
                  </span>
                  /5
                </TableCell>

                <TableCell className="py-4 pl-4.5 pr-6 text-center">
                  <PriceCell price={product.price} />
                </TableCell>

                <TableCell className="py-4 pl-4.5 pr-6">
                  <div className="flex items-center gap-2 justify-start">
                    <Button className="w-13 h-6.75 rounded-full bg-[#3B4FF5] hover:bg-[#2D3FE0]">
                      <Plus className="h-4 w-4" />
                    </Button>
                    <Button className="w-9 h-9 rounded-full border bg-white">
                      <MoreHorizontal className="h-4 w-4 text-gray-400" />
                    </Button>
                  </div>
                </TableCell>
              </TableRow>
            );
          })
        )}
      </TableBody>
    </Table>
  );
};
